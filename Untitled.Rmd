...
plot(log10(sub$Raw), sub$Frac.RNA, xlab='Log10 No. of Raw Reads', ylab='cDNA concentration (ng/uL)', pch =21, bg='black')
...
```{r 18s-asvs-frac, fig.width=9, fig.height=7, dpi=300}
sub=subset_samples(phyo_abund, Comm_Frac=="Frac")
...
Note had to take away samples that had less than 100 reads 
...
##### Acetate
```{r bdshift2, message=FALSE, error=FALSE, warning=FALSE, eval=FAlSE}

wmean1 = BD_shift(physeq_S2D2_l[[1]], nperm=20,ex="Treatment=='Control'", perm_method='overlap')
cat('Subset:', names(physeq_S2D2_l)[1], '\n')
...
ggplot(wmean1_m, aes(BD_min.x, wmean_dist)) +
  ...
  geom_point(aes(color=BD_shift)) +
  scale_color_discrete('Gradient\nfraction\nin BD shift\nwindow?') +
  labs(x=x_lab, y=y_lab, title='Beta diversity of 13C-Acetate relative to 12C-Con') +
  theme_bw() 
...
According to my settings, there are 11 incorporators 

```{r, message=FALSE, error=FALSE, warning=FALSE}

phyo_frac=subset_samples(phyo2, pch=="22")
